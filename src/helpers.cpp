...
void helpers::filterCollsionGeometry(drake::geometry::SceneGraph<double> &scene_graph, drake::systems::Context<double> *context)
{
    ...
    drake::log()->info("Filtered collision pairs from " + std::to_string(inspector.) +  " to " + std::to_string(pairs.size()));
}
...
//     // res["top_shelf/top_shelf"] = Eigen::Matrix<double, 2, 7>(a, b, c, d, e, f, g,
//     //                                                          a, b, c, d, e, f, g);
...
std::unordered_map<std::string, std::vector<double>> helpers::getConfigurationSeeds()
{
    std::unordered_map<std::string, std::vector<double>> seeds = {
